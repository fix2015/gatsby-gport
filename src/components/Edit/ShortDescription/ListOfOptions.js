...
export default function ShortDescription({ options, onCallback }) {
  ...
  const [optionParams, setOptionParam] = useState(generateEditOptionsIcons(options))

  const handleChange = (event) => {
    if (!event.target.checked) {
      setOptionSaved(optionSaved.filter((option) => option.icon !== event.target.name))
    } else {
      const {name, icon} = optionsIcons.filter((optionIcon) => optionIcon.icon === event.target.name)[0];
  ...
  return (
    <FormGroup row>
      {optionParams.map(({ name, icon, Component, value }, ind) => (
        <FormControlLabel
          key={ind}
          control={<Switch name={icon} onChange={handleChange} checked={value}
